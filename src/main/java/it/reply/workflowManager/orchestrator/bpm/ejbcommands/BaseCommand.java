...
public abstract class BaseCommand implements IEJBCommand {
	...
	public ExecutionResults execute(CommandContext ctx) throws Exception {
		...
		do {
			...
			try {
				...
				while (cause != null) {
					if (cause instanceof PersistenceException
						...
						cause = !cause.equals(cause.getCause()) ? cause.getCause()
								: null;
					}
				}
			...
			SignalEvent<?> signalEvent = (SignalEvent<?>) exRes
	...
	protected void logCommandRetry(final ExecutionResults exResults,
		...
		try {
			...
			sb.append("RETRY, Try ").append(tryNum).append(" of ")
					.append(maxNumOfTries).append(".");
			@SuppressWarnings("unchecked")
			SignalEvent<Error> signalEvent = (SignalEvent<Error>) exResults.getData(Constants.SIGNAL_EVENT);
	...
	protected ExecutionResults handlePersistenceException(Exception ex,
	...
	protected ExecutionResults errorOccurred(Error error,
			ExecutionResults exResults) {
		SignalEvent<Error> signalEvent = new SignalEvent<Error>(SignalEvent.SignalEventType.ERROR,
