...
@Deprecated
public class CustomJPAPlaceholderResolverStrategy implements ObjectMarshallingStrategy, TransactionAware, Cacheable {
    @SuppressWarnings("unused")
	private static Logger log = LogManager.getLogger(CustomJPAPlaceholderResolverStrategy.class);
...
//    		throw new UnsupportedOperationException("Command parameters must not have inner entities. Entities must only be directly contained by the map.");
//    	}
    	return containsEntities(object);
        //return false;
    }

    private <T> void setIdAsProperty(T entity, T fakeEntity, Method getterMethod) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    private <T> T wrapEntity(T entity, Object id) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    private <T> T wrapEntities(T object, EntityManager em) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    	if (object == null) {
    		return object;
    	}
    	if (isEntity(object)) {
    ...
    @SuppressWarnings("unchecked")
	private <T> T unwrapEntities(T object, EntityManager em) throws IllegalArgumentException, IllegalAccessException{
    	if (object == null) {
    		return object;
    	}
    	if (isEntity(object)) {
            ...
            for (Field field : varClass.getDeclaredFields()) {
                ...
                if (!Modifier.isStatic(mod) && !Modifier.isTransient(mod) && !field.isEnumConstant()) {
	    			...
	    			try {
						...
						throw new RuntimeException("Error trying to understand if " + o.toString() + " is an entity.");
					}
                }
            }
        } while ((varClass = varClass.getSuperclass()) != null);
